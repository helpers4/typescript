...
/**
 * Test source category structure: no unexpected subdirectories.
 *
 * Note: index.ts is auto-generated at build time (gitignored) — re-export
 * completeness is guaranteed by the build step, no need to check it here.
 */
export async function testSourceStructure(): Promise<void> {
  ...
  for (const category of categories) {
    ...
    // Flag unexpected subdirectories
